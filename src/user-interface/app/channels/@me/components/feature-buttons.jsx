...
export const FeatureButton = ({ name }) => {
  const navigate = useNavigate();
  const icon = { Friends: friendsSvg, Nitro: nitroSvg, Shop: shopSvg };

  const handleRedirect = () => {
    if (name === 'Friends') navigate('/channels/@me');
  };

  return (
    <button
      className='flex group py-[10px] px-3 gap-4 font-semibold rounded-md text-[#999] hover:bg-[#33373b] hover:text-[#ddd] active:text-[#eee]'
...
const friendsSvg = (
  <svg
    className='linkButtonIcon_c91bad text-[#999] group-hover:text-[#ddd] group-active:text-[#eee]'
    aria-hidden='true'
    role='img'
